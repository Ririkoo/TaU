...
class CometModel(ptl.LightningModule, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        self.save_mcd = False  # Flag used to save MC Dropout runs
        self.caching = False  # Flag used to control Embedding Caching
        self.test_optimizer = None
        self.post_infer = False

        # If not defined here, metrics will not live in the same device as our model.
        self.init_metrics()

    def set_test_optimizer(self, optim:torch.optim):
        self.test_optimizer = optim

    def set_post_infer(self, post_infer:bool):
    ...
    def predict_step(
        ...
        Return:
            Predicion object
        """
        if self.test_optimizer is not None:
            assert self.mc_dropout > 0  # require MCD
            with torch.inference_mode(False):
                ...
                if self.post_infer:
                    with torch.no_grad():
                        self.eval()
                        final_score = self(**batch).score
                    self.train()

                else:
                    final_score = mcd_mean

                return Prediction(
                    scores=final_score,
                    metadata=Prediction(mcd_std=mcd_std, mcd_outputs=mcd_outputs.detach()),
                )
            ...
            if self.save_mcd:
                return Prediction(
                    scores=mcd_mean,
                    metadata=Prediction(mcd_std=mcd_std, mcd_outputs=mcd_outputs),
                )
            return Prediction(scores=mcd_mean, metadata=Prediction(mcd_std=mcd_std))
        ...
        def flatten_metadata(metadata):
            ...
            for k, v in metadata.items():
                if torch.is_tensor(v[0]):
                    ...
                    if k == "mcd_outputs":
                        metadata[k] = torch.cat(v, dim=1).T.tolist()
                    else:
    ...
    def predict_adapt(
        ...
        test_optimizer: torch.optim = None,
    ) -> Prediction:
        """Method that receives a list of samples (dictionaries with translations,
        sources and/or references) and returns segment-level scores, system level score
        and any other metadata outputed by COMET models. If `mc_dropout` is set, it
        ...
        if test_optimizer is None:
            ...
            self.set_test_optimizer(test_optimizer)
            self.set_post_infer(post_infer)
